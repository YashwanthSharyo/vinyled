...
const Header = () => {
  ...
  const toggleMenu = () => {
    setIsMenuOpen(!isMenuOpen);
    if (isMenuOpen) {
      setActiveDropdown(null); // Close dropdowns when menu is closed
    }
  };

  const handleMenuItemClick = (path) => {
    setIsMenuOpen(false);
    setActiveDropdown(null); // Close dropdowns on item selection
    navigate(path, { replace: true });
    window.scrollTo(0, 0);
  };
  ...
  useEffect(() => {
    const handleOutsideClick = (e) => {
      if (window.innerWidth <= 1000) { // Changed: Align with 1000px breakpoint
